...
function speak(text, rate = 0.45) {
  // Pad with a comma to create a natural pause at the start
...
export default function MorphAnimation() {
  ...
  const speechTimersRef = useRef([]); // track scheduled speech timeouts
  ...
  const switchState = useCallback((newState) => {
    ...
    if (newState === "breathing") {
      speak("Fully in, let it go", 0.4);
      speechTimersRef.current.push(
        setTimeout(() => speak("Fully in, let it go", 0.4), 4000),
        setTimeout(() => speak("Fully in, let it go", 0.4), 8000),
        setTimeout(() => speak("Keep going", 0.45), 12000),
      );
    }
    if (newState === "retention") speak("Breathe in deeply one more time, let it go, and wait for as long as you feel comfortable", 0.4);
    if (newState === "recovery") speak("Breathe in deeply, and hold, 15 seconds", 0.4);
    ...
    function draw(timestamp) {
      ...
      if (phase === "transition") {
        ...
        if (elapsed >= maxElapsed) {
          ...
          speechTimersRef.current.forEach(clearTimeout);
          speechTimersRef.current = [];
          speak("Breathe in deeply one more time, let it go, and wait for as long as you feel comfortable", 0.4);
        } else {
        ...
        if (elapsed < 15000) {
          ...
          if (scaleRef.current === 1) {
            // First frame of shrink - say "now let go"
            speechSynthesis.cancel();
            speak("Now let go", 0.4);
          }
      ...
      // Recovery countdown
      if (state === "recovery" && phase === "hold" && elapsed < 15000) {
